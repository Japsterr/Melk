...
function TfrmInentKoei.vindKoei;
var id : String;
    found : Boolean;
...
function TfrmInentKoei.findCattle;
var id : String;
    found : Boolean;
    SearchOptions : TLocateOptions;
begin
...
procedure TfrmInentKoei.FormCreate(Sender: TObject);
begin
  edtDate.DateTime := Now();
end;

procedure TfrmInentKoei.open;
begin
  edtId.Enabled := false;
...
procedure TfrmInentKoei.btnAddClick(Sender: TObject);
begin
  insert;
  close;
  DataModule1.connect;
  ShowMessage('Die Koei se inenting is suksesvol in die stelsel gelaai.');
end;

procedure TfrmInentKoei.btnVindClick(Sender: TObject);
begin
  if findCattle then
  Begin
    if vindKoei then
  ...
  End
    ...
    ShowMessage('Daar is nie n bees in die stelsel met die id / tag nie.');
end;

procedure TfrmInentKoei.close;
begin
  edtId.Enabled := true;
  edtId.Clear;
  edtId.SetFocus;
...
procedure TfrmInentKoei.insert;
